...
(deftest block-indentation
  (testing "with default config"
    (is (reformatted?
          indent/reindent {:indents {'if [[:block 1]]}}
          "(if (= x 1)\n:foo\n:bar)"
          "(if (= x 1)\n  :foo\n  :bar)"))
    (is (reformatted?
  ...
  (testing "with list-indent 1"
    ...
    (is (reformatted?
        indent/reindent {:indents {'do [[:block 0]]}
          ...
          "(do\n  (foo)\n  (bar))"))
    (is (reformatted?
          indent/reindent {:indents {'deftype [[:block 1] [:inner 1]]}
                           :list-indent 1}
          "(deftype Foo\n[x]\nBar)"
          "(deftype Foo\n  [x]\n  Bar)")))
  (testing "with custom indent rule"
    (is (reformatted?
          indent/reindent {:indents {'assoc [[:block 1 2]]}}
          "(assoc {} :foo bar\n:foo2 bar2)"
          "(assoc {} :foo bar\n          :foo2 bar2)"))
    (is (reformatted?
          indent/reindent {:indents {'assoc [[:block 1 2]]}}
...
(deftest stair-indentation
  (let [indents {'cond [[:stair 0]]
                 ...
                 'cond->> [[:stair 1]]}]
    (is (reformatted?
          indent/reindent {:indents indents}
          "(cond\na? a\n   b? b)"
          "(cond\n  a? a\n  b? b)"))
    (is (reformatted?
          indent/reindent {:indents indents}
...
(deftest constant-indentation
  (is (reformatted?
        indent/reindent {:indents {'def [[:inner 0]]}}
        "(def foo\n\"Hello World\")"
        "(def foo\n  \"Hello World\")"))
  (is (reformatted?
        indent/reindent {:indents {'defn [[:inner 0]]}}
        "(defn foo\n[x]\n(+ x 1))"
        "(defn foo\n  [x]\n  (+ x 1))"))
  (is (reformatted?
        indent/reindent {:indents {'defn [[:inner 0]]}}
  ...
  (is (reformatted?
        ...
        "(defn foo\n  ([] 0)\n  ([x]\n   (+ x 1)))"))
  (is (reformatted?
        indent/reindent {:indents {'fn [[:inner 0]]}}
        "(fn [x]\n(foo bar\nbaz))"
        "(fn [x]\n  (foo bar\n       baz))"))
  (is (reformatted?
        indent/reindent {:indents {'fn [[:inner 0]]}}
...
(deftest inner-indentation
  (is (reformatted?
        indent/reindent {:indents {'letfn [[:block 1] [:inner 2 0]]}}
        "(letfn [(foo [x]\n(* x x))]\n(foo 5))"
        "(letfn [(foo [x]\n          (* x x))]\n  (foo 5))"))
  (is (reformatted?
        indent/reindent {:indents {'letfn [[:block 1] [:inner 2 0]]}
...
(deftest embedded-structures
  (is (reformatted?
        indent/reindent {:indents {'let [[:block 1]]}}
        "(let [foo {:x 1\n:y 2}]\n(:x foo))"
        "(let [foo {:x 1\n           :y 2}]\n  (:x foo))"))
  (is (reformatted?
        indent/reindent {:indents {'let [[:block 1]]}}
